...
object niveles {
	...
	method getListOfEnemies() = (1 .. self.numberOfEnemies()).fold([], { enemies , value =>
		enemies.add(self.createNewEnemy(value))
		return enemies
	})
...
//    }
	method createNewEnemy(number) {
		return new Enemigo(direction = self.getRandomMovementType(), numeroEnemigo = number, // multiverse = random.up(1,3),
		multiverse = new Range(start = 1, end = 3).anyOne(), mposition = game.at(new Range(start = 1, end = 10).anyOne(), new Range(start = 1, end = 10).anyOne())// mposition = game.at(random.up(0,12), random.up(0,12))
		)
	}

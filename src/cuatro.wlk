...
object cuatro {
	...
	method ponerFicha(jugador) {
		...
		const ficha = new Ficha(player = jugador, position = pos, image = jugador.grabed().imagenFicha())
	...
	method getVisualsColumnaActual(posX, posY) {
		return fichasJugadas.filter{ ficha => ficha.position().x() == posX and ficha.position().y().between(0, posY + 3) }
	}
	...
	method seResuelveCon(ficha, jugador) = self.gano(ficha, jugador, 1, 0) or self.gano(ficha, jugador, 0, 1) or self.gano(ficha, jugador, 1, 1) or self.gano(ficha, jugador, 1, -1)
	...
	method jugadaGanadora(jugador) {
		self.fichasJugadasPor_(jugador).forEach{ ficha =>
			if (self.seResuelveCon(ficha, jugador)) {
				...
				resuelto = true
				game.schedule(3000, { self.EliminarTodasLasFichas()
					omniverse.current(6)
				})
					// space stop
	...
	method estanLasFichasConPosiciones(jugador, posUno, posDos, posTres, posCuatro) {
		return self.fichasJugadasPor_(jugador).filter{ ficha => ficha.position() == posUno or ficha.position() == posDos or ficha.position() == posTres or ficha.position() == posCuatro }.size() == 4
	}

	method gano(ficha, jugador, sx, sy) {
		return self.estanLasFichasConPosiciones(jugador, game.at(ficha.position().x(), ficha.position().y()), game.at(ficha.position().x() + (1 * sx), ficha.position().y() + (1 * sy)), game.at(ficha.position().x() + (2 * sx), ficha.position().y() + (2 * sy)), game.at(ficha.position().x() + (3 * sx), ficha.position().y() + (3 * sy)))
	}

...
class Ray inherits OmniObjeto {
	...
	method next() {
		game.schedule(300, { mposition = direction.nextPosition(mposition)
			self.start()
		})
	}
...
object portalgun mixed with Collectable {
	...
	method crearPortalA(multiverseDestino, direction) {
		const portal = new Portal(mposition = rick.direction().nextPosition(position), multiverse = multiverse, exit = new Portal(mposition = rick.direction().nextPosition(position), multiverse = multiverseDestino, exit = null))
...
class Portal inherits OmniObjeto mixed with NotCollectable {
	...
	method travel(traveler) {
		...
		game.schedule(1700, { game.removeVisual(self)
